...
class MessageMaker(object):
    ...
    def __init__(self):
        ...
        self.STATE_KOR = {"HR": "홈런", "HIT": "안타", "H2": "2루타", "H3": "3루타", "BB": "볼넷", "SO": "삼진", "RBI": "타점",
    ...
    def get_pitcher_message(self, parameters):
        return None

    def get_hit_event_message_new(self, parameters):
        selected_param_list = []
        usable_list = []
        sentence = ''
        ...
        for param in selected_param_list:
            param_dict = param[4]
            ...
            if state_split == "NGAME" or state_split == "NPA":
                usable_list.append({state_split: {'result': int(state_result), 'state': self.STATE_KOR[state]}})
            ...
            # region Versus Pitcher and Team
                ...
                if pos_neg == "POS":
                    record_good_bad = '높은'
                    usable_list.append({'STYLE_POS': {'state': self.STATE_KOR[state]}})
                else:
                    record_good_bad = '낮은'
                    usable_list.append({'STYLE_NEG': {'state': self.STATE_KOR[state]}})

                # usable_list.append({league_data: {'league_data': league_data}})
                state_word = "{0} {1}개로 ".format(self.STATE_KOR[state], int(state_result))
                usable_list.append({'STATE': {'state': state_word}})

                if league_data == "ALL":
                    usable_list.append({'COMPARE_ALL': {'percent': rate_score,
                                                        'result': record_good_bad, 'state': self.STATE_KOR[state]}})
                else:
            ...
            # region 점수차, 주자상황 Split
                ...
                if pos_neg == "POS":
                    record_good_bad = '높은'
                    usable_list.append({'GOOD_RECORD': {'state': self.STATE_KOR[state]}})
                else:
                    record_good_bad = '낮은'
                    usable_list.append({'BAD_RECORD': {'state': self.STATE_KOR[state]}})
                if state in self.FLOAT_STATE:
                    hal_pun_li = self.get_halpunli(state_result)
                    state_word = "{state} {rate:로} ".format(state=self.STATE_KOR[state], rate=Noun(hal_pun_li))
                else:
                    state_word = "{0} {1}개로 ".format(self.STATE_KOR[state], int(state_result))
                usable_list.append({'STATE': {'state': state_word}})
            ...
            if usable_list:
                sentence += re.sub("\s\s+", " ", self.get_template_sentence('HITTER_SPLIT', usable_list))
                sentence += '\n'
                usable_list = []
        return sentence
    ...
    def get_hitter_split_message(self, parameters):
        group_id = 'HITTER_SPLIT'
        positive_count = 0
        ...
        for league in league_list:
            ...
            for param in parameters:
                ...
                if league == data_dict['LEAGUE']:
                    ...
                    if state in self.FLOAT_STATE:
                        ...
                        if state_result:
                            state_list.append('{0}{1}'.format(self.STATE_KOR[state], state_result))
                            if pos_neg == "POS":
                        ...
                        if state_result == 0 and state == "HIT":
                            state_list.append('무{0}'.format(self.STATE_KOR[state]))
                        elif state == "RBI":
                            if pos_neg == "POS":
                                state_pos_list.append(state)
                            else:
                                state_neg_list.append(state)
                            state_list.append('{0} {1}'.format(state_result, self.STATE_KOR[state]))
                        elif state_result > 0:
                            if pos_neg == "POS":
                                state_pos_list.append(state)
                            else:
                                state_neg_list.append(state)
                            state_list.append('{0} {1}개'.format(self.STATE_KOR[state], state_result))
            ...
            if state_count > 0:
                ...
                if split == 'NPA' or split == 'NGAME':
                    usable_list.append({league: {'league': league}})
                    usable_list.append({split: {'result': state_result, 'state': self.STATE_KOR[state]}})
                # endregion
                else:
                    ...
                    if state_count > 1:
                        if state_pos_list:
                            state_pos_name = ', '.join([self.STATE_KOR[x] for x in state_pos_list])
                        if state_neg_list:
                            state_neg_name = ', '.join([self.STATE_KOR[x] for x in state_neg_list])
                    ...
                    if hra > 0:
                        ...
                        state_list.append('{0} {1}'.format(self.STATE_KOR['HRA'], halpunli))
    ...
    def get_today_event_message(self, parameters):
        state_dict = {}
        usable_list = []
        hitter_name = ''
        league = ''
        ...
        for param in parameters:
            ...
            # region TODAY SPLIT
            if event_split == "TODAY_TODAY":
                if 'PA' not in state_dict:
                    state_dict['PA'] = "{0}{1}".format(pa, self.STATE_KOR['PA'])
                if state in self.FLOAT_STATE:
                    hal_pun_li = self.get_halpunli(state_result)
                    if hal_pun_li:
                    ...
                    if state_val == 0 and state == "HIT":
                        ...
                        state_dict[state] = "{0}{1}".format(state_val, self.STATE_KOR[state])
            # endregion TODAY SPLIT
            # region FIRST SPLIT
            elif event_split == "TODAY_FIRST":
                if state in self.FLOAT_STATE:
                    state_result = self.get_halpunli(data_dict['RESULT'])
        ...
        if state_count > 0:
            ...
            sentence = self.get_template_sentence('TODAY', usable_list)
    ...
    def get_today_basic_event_message(self, parameters):
        state_dict = {}
        usable_list = []
        hitter_name = ''
        sentence = ''
            ...
            for param in parameters:
                ...
                if league == league_data:
                    ...
                    if state in self.FLOAT_STATE:
                        hal_pun_li = self.get_halpunli(state_result)
                        if hal_pun_li:
                            state_list.append({'state': self.STATE_KOR[state], 'rank': rank, 'result': hal_pun_li})
                    else:
                        state_val = int(state_result)
                        state_list.append({'state': self.STATE_KOR[state], 'rank': rank, 'result': state_val})

                state_count = len(state_dict)
                if state_count > 0:
                    usable_list.append({'HITNAME': {'hitname': hitter_name}})
                    usable_list.append({league: {'league': league}})
                    usable_list.append({'BASIC': state_list})
                    usable_list.append({'BASIC_FINISH': {'league': league}})

            if usable_list:
                sentence += re.sub("\s\s+", " ", self.get_template_sentence('TODAY', usable_list, hitter))
                usable_list = []
    ...
    def get_common_message(self, parameters):
        state = parameters[0][2]
        param_dict = parameters[0][4]
        sentence = ''
        template_dict = MessageUnit.get_template_dict('COMMON')
        ...
        if state in template_dict:
            ...
            if state == "GAMEINFO":
                ...
                sentence = template.format(stadium=param_dict['STADIUM'], hteam=hteam, vteam=vteam,
                ...
                sentence = template.format(**param_dict)
            elif state == "0T":
                sentence = template
            elif state == "0S":
                sentence = template
            elif state == "CURRENT_INFO":
                sentence = template.format(**param_dict)
            elif state == "FULL_COUNT":
                sentence = template
            elif state == "CONTINUE_BALL":
                sentence = template.format(**param_dict)
            elif state == "BALL_STUFF_COUNT":
                sentence = template.format(**param_dict)
            return sentence
